...
### 1.2 安全特性 (2026-05-17 更新)

本版本包含最新的安全加固措施：

| 安全特性 | 说明 |
|---------|------|
| **主密钥验证** | 文件权限≤0600、符号链接检测、来源审计 |
| **API输入限制** | 请求体大小限制、字段长度校验、CSR格式验证 |
...
### 1.3 私钥导出审批流程 (2026-05-17 更新)

本版本新增完整的私钥导出审批系统：

| 功能 | 说明 |
|------|------|
| **导出申请** | 申请人填写导出原因和加密密码，提交审批申请 |
| **多级审批** | 需 2 名 SEC_ADMIN 审批通过（可配置） |
| **有效期** | 审批通过后 24 小时内有效，过期需重新申请 |
| **二次认证** | 执行导出时需验证当前登录密码 |
| **加密导出** | 支持 PBKDF2(60万轮)+AES-256-GCM 加密导出 |
| **明文导出** | 支持留空密码明文导出（带安全警告） |
| **限额控制** | 日限额 + 单密钥限额原子化计数 |
| **审计追踪** | 导出操作记录 CRITICAL 级别审计日志 |

前端操作路径：
1. 证书管理 → 点击"导出私钥" → 填写二次认证密码和加密密码 → 确认
2. 若配置需要审批，系统自动提交申请，跳转至"导出审批"页面
3. SEC_ADMIN 在"导出审批"页面审批通过/拒绝
4. 申请人在"导出审批"页面点击"执行导出"完成下载

### 1.4 证书功能增强 (2026-05-17 更新)

本版本新增证书功能：

| 功能 | 说明 |
|------|------|
| **国密扩展支持** | 支持国密身份标识扩展(OID 1.2.156.112562.2.1.1.23)和国密特有扩展(OID 2.16.840.1.113732.5) |
| **CRL分发点** | 支持CRL分发点扩展(OID 2.5.29.31)，启用证书吊销验证 |
| **Netscape扩展** | 支持Netscape证书类型扩展，提高浏览器兼容性 |
| **完整主题字段** | 支持State/Province和Locality字段，证书信息更完整 |
| **国密应用兼容** | 提高GM/T 0015标准兼容性，支持国密应用场景 |

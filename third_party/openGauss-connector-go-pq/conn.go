...
func (cn *conn) auth(r *readBuf) {
	...
	case AuthReqSha256:
		...
		if passwordStoredMethod == PlainPassword || passwordStoredMethod == Sha256Password {
			...
			// openGauss 6.x 的 AuthenticationSHA256 报文为
			// random64code(64) + token(8) + server_signature(64)，并无 iteration
			// 字段，PBKDF2 迭代次数固定为 10000。旧实现误用 r.int32() 把
			// server_signature 的前 4 字节当二进制整数读取，导致迭代次数被放大
			// 到数亿次，连接时 CPU 狂算。
			serverSignature := string(r.next(64))
			result := RFC5802Algorithm(cn.config.Password, random64code, token, serverSignature, 10000, "sha256")
			if len(result) == 0 {
